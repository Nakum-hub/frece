...
def render_html_report(report: dict, case_name: str) -> str:
    ...
    # Escape every evidence-derived value: artifact types/categories can contain
    # attacker-controlled strings, and this report is opened in a browser.
    type_rows = "".join(
        f"<tr><td>{html.escape(str(t))}</td><td>{int(c)}</td></tr>" for t, c in top_types
    )
    cat_rows = "".join(
        f"<tr><td>{html.escape(str(c))}</td><td>{int(cnt)}</td></tr>"
        for c, cnt in sorted(category_counts.items(), key=lambda x: -x[1])
    )

    safe_case = html.escape(str(case_name))
    safe_generated = html.escape(str(report.get("generated_at", "")))
    safe_custody = html.escape(str(custody_ok))
    safe_entries = html.escape(str(report.get("custody_entries", 0)))
...
<title>FRECE Case Report — {safe_case}</title>
<style>
...
<div class="header">
  ...
  <p>Case: <strong>{safe_case}</strong> &nbsp;·&nbsp; Generated: {safe_generated} &nbsp;·&nbsp; Chain of Custody: {safe_custody}</p>
</div>
<div class="container">
  <div class="grid">
  ...
  <div class="section">
    ...
    <p>Entries: <strong>{safe_entries}</strong> &nbsp;·&nbsp; Status: <span class="badge {badge_cls}">{safe_custody}</span></p>
  </div>
</div>
